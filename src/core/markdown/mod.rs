...
use crate::core::document::{
    DirectiveKind, Document, DocumentElement, DocumentParameters, TableAlignment,
};
        ...
        match self.phase {
            ...
            FrontmatterPhase::Complete => {
                ...
                if self.doc.parameters.header {
                    ...
                    let subtitle = (!self.doc.parameters.subtitle.trim().is_empty())
                        .then(|| self.doc.parameters.subtitle.clone());
                    let logo = (!self.doc.parameters.logo.trim().is_empty())
                        .then(|| self.doc.parameters.logo.clone());
...
mod tests {
    use super::*;
    use crate::core::document::{DocumentHeader, ShoutoutElementKind};
