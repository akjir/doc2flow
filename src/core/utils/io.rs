...
impl<T> IoErrorExt<T> for std::result::Result<T, std::io::Error> {
    fn with_path(self, path: &Path) -> Result<T> {
        self.map_err(|err| Error::Message(format!("I/O error at {}: {err}", path.display())))
    }
}
...
/// # Errors
///
/// Returns [`Error`] if metadata cannot be queried for the target path.
...
/// # Errors
///
/// Returns [`Error`] if directory deletion fails.
...
mod tests {
    ...
    impl TestTempDir {
        fn new(prefix: &str) -> Self {
            let path = std::env::temp_dir().join(format!("d2f_test_core_{prefix}_{}", std::process::id()));
    ...
    fn test_io_non_existent_file_errors() {
        ...
        assert!(err_str.to_string().contains(missing));

        let err_bytes = read_file_bytes(missing).unwrap_err();
        assert!(err_bytes.to_string().contains(missing));
    }
