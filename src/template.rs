...
/// Application version dynamically extracted from Cargo.toml at compile time.
...
mod tests {
    ...
    fn test_render_metadata_injection() {
        ...
        assert!(html.contains(&format!("Generated by Doc2Flow (d2f) v{}", APP_VERSION)));
        assert!(html.contains(&format!("Repository: {}", REPOSITORY_URL)));
        assert!(html.contains(&format!("License: {}", LICENSE_TERMS)));
        assert!(html.contains(&format!("<meta name=\"generator\" content=\"Doc2Flow v{} ({})\">", APP_VERSION, REPOSITORY_URL)));
        assert!(html.contains(&format!("<meta name=\"repository\" content=\"{}\">", REPOSITORY_URL)));
        assert!(html.contains(&format!("<meta name=\"license\" content=\"{}\">", LICENSE_TERMS)));
        assert!(!html.contains("{{APP_VERSION}}"));
        assert!(!html.contains("{{REPOSITORY_URL}}"));
        assert!(!html.contains("{{LICENSE_TERMS}}"));
    }
}
