...
/// assert_eq!(features[1].name(), "fields");
/// assert_eq!(features[2].name(), "header");
/// assert_eq!(features[3].name(), "images");
/// assert_eq!(features[4].name(), "tables");
/// assert_eq!(features[5].name(), "tasks");
/// ```
pub fn get_all_features() -> Vec<Box<dyn Feature>> {
    vec![
        Box::new(CodeFeature::new()),
        Box::new(FieldsFeature::new()),
...
mod tests {
    ...
    fn test_feature_registry_contains_registered_features() {
        ...
        assert_eq!(list[1].name(), "fields");
        assert_eq!(list[2].name(), "header");
        assert_eq!(list[3].name(), "images");
        assert_eq!(list[4].name(), "tables");
        assert_eq!(list[5].name(), "tasks");
    }
}
