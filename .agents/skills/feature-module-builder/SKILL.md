...
1. **Scaffold Slice:** Create `src/features/<name>/` directory:
   - `module.rs`: Feature struct `<Name>Feature` implementing `Feature` trait (`name`, `is_enabled`, `javascript`, `css`), `new()`, and unit tests.
...
4. **Enforce Directives (`AGENTS.md`):**
   - **Rust:** Zero `unsafe`, zero-alloc hot path, `#[inline]`, canonical doc headers (Examples/Errors/Panics), Stdlib+`Doc2FlowError` (no `anyhow`/`eyre`).
