...
"#;
    ...
    // Verify HTML comment header
    assert!(rendered.contains("<!--\n===============================================================================\nGenerated by Doc2Flow (d2f) v"));
    assert!(rendered.contains(doc2flow::template::APP_VERSION));
    assert!(rendered.contains("Repository: https://github.com/akjir/doc2flow"));
    assert!(rendered.contains("License: GPL-3.0-or-later"));

    // Verify meta tags
    assert!(rendered.contains(&format!("<meta name=\"generator\" content=\"Doc2Flow v{} (https://github.com/akjir/doc2flow)\">", doc2flow::template::APP_VERSION)));
    assert!(rendered.contains("<meta name=\"repository\" content=\"https://github.com/akjir/doc2flow\">"));
    assert!(rendered.contains("<meta name=\"license\" content=\"GPL-3.0-or-later\">"));

    // Verify template.md metadata comments
    let init_tmpl = doc2flow::template::generate_template_markdown();
    assert!(init_tmpl.contains(&format!("DOC2FLOW (D2F) v{} - TEMPLATE & USAGE GUIDE", doc2flow::template::APP_VERSION)));
