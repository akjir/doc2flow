...
* **Programming Language:** Rust (Edition 2024) for CLI backend, TypeScript 7.0 for client-side toolchain.
...
doc2flow/
├── .cargo/
│   └── config.toml           # Cargo Aliases (e.g. cargo win / Cross-Compile)
...
│   ├── package.json          # Node/Bundler Config (esbuild/typescript)
│   ├── tsconfig.json         # TS compiler options
│   ├── src/
│   │   ├── core/             # Base features (Always included in HTML)
│   │   │   ├── collapse.ts   # Collapse / Expand H1/H2
│   │   │   ├── search.ts     # Content search & auto-expand matches (Ctrl+K)
│   │   │   ├── storage.ts    # Central storage (status of highlights, states)
│   │   │   └── main.ts       # Core entry (initialization of collapse, search & storage)
│   │   └── features/         # Modular optional extension components
│   │       ├── tasks.ts      # Checkboxes, LocalStorage sync, progress bar & reset
│   │       ├── images.ts     # Lightbox & zoom functionality on image click
│   │       └── toc.ts        # Dynamic / Interactive Table of Contents
│   └── dist/                 # JS files separately generated by esbuild
│       ├── script-core.js    # Bundles core/
│       ├── script-tasks.js   # Bundles features/tasks.ts
│       ├── script-images.js  # Bundles features/images.ts
│       └── script-toc.js     # Bundles features/toc.ts
├── src/                      # Rust Backend/CLI Source Code
│   ├── main.rs               # CLI entry point & argument parsing
│   ├── lib.rs                # Module exports
│   ├── io.rs                 # Filesystem, path resolution, asset loading
│   ├── error.rs              # Central error handling & DiagnosticError
│   ├── converter.rs          # Markdown AST parser & feature detection
│   ├── template.rs           # Template engine & dynamic script assembly
│   ├── components.rs         # HTML UI components (badges, buttons, forms)
│   └── i18n.rs               # HashMap i18n loader & dynamic substitution
├── tests/
│   └── integration_test.rs   # Integration tests for CLI & HTML output
├── build.rs                  # Builds TS files individually to web/dist/ & Git Hash
├── Cargo.toml                # Rust dependencies & profiles
├── SPECIFICATION.md          # Functional specification
├── AGENTS.md                 # Guidelines for AI coding systems
